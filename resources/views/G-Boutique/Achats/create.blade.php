...
@section('contenu')
    <div class="container-fluid px-4">
        <h1 class="mt-4">Ravitaillement en produit (stock)</h1>
        <div class="card w-100">
            <div class="card-header bg-primary d-flex justify-content-between align-items-center text-white">
                <h4 class="row">Achats</h4>
                <div class="btn-group">
                    <a href="{{ route('achats.index') }}" class="btn btn-light btn-sm">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('ajouterAu.panier') }}" method="POST">
                                @csrf
                                <div class="row mb-3">
                                    <div class="col-md-12">
                                    ...
                                    <div class="col-md-4">
                                        <label for="prix_achat" class="form-label">P.U achat</label>
                                        <input type="number" id="prix_achat" class="form-control" name="prix_achat"
                                            value="{{ old('prix_achat') }}" placeholder="Entrer le prix">
                                    </div>
                                    <div class="col-md-4">
                                        <label for="prix_vente" class="form-label">P.U vente</label>
    ...
    <table class="table" id="tablePanier">
        ...
        <tbody>
            @if (session('panier_achat'))
                @foreach (session('panier_achat') as $id => $item)
                    @php
                        $montantAchat = $item['prix_achat'] * $item['quantite'];
                        $montantVente = $item['prix_vente'] * $item['quantite'];

                        $totalQuantite += $item['quantite'];
                        $totalMontantAchat += $montantAchat;
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($item['prix_vente'], 0, '.', ' ') }}</td>
                        {{-- <td>{{ number_format($montantAchat, 0, '.', ' ') }}</td>
                        <td>{{ number_format($montantVente, 0, '.', ' ') }}</td> --}}
                        <td>
    ...
    <tr>
        ...
        <th>{{ number_format($totalQuantite > 0 ? $totalMontantAchat / $totalQuantite : 0, 0, '.', ' ') }} F</th>
        <th>{{ number_format($totalQuantite > 0 ? $totalMontantVente / $totalQuantite : 0, 0, '.', ' ') }} F</th> --}}
        <th>{{ number_format($totalMontantAchat, 0, '.', ' ') }} F CFA</th>
        <th>Total vente</th>
        <th>{{ number_format($totalMontantVente, 0, '.', ' ') }} F CFA</th>
        <th></th>
    </tr>
</tfoot>
    </table>
                ...
                <div class="col-md-12"><br>
                    ...
                    <div class="card">
                        <div class="card-header bg-dark mb-3 text-white">Finaliser l'achat</div>
                        <div class="card-body">
                            <form id="formSubmitPanier" method="POST" action="{{ route('achats.store') }}">
                                @csrf
                                <div class="row mb-3">
                                    <div class="input-group mb-3">
                                        <div class="flex-grow-1">
                                            <select id="fournisseur_id" name="fournisseur_id"
                                                class="form-select select2 @error('fournisseur_id') is-invalid @enderror">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <label for="date_achat" class="form-label">Date</label>
                                            <input type="date" class="form-control @error('date_achat') is-invalid @enderror"
                                                name="date_achat" value="{{ old('date_achat') }}">
                                            @error('date_achat')
                                                <span class="invalid-feedback">{{ $message }}</span>
                                            @enderror
                                        </div>
                                ...
                                <div class="card-footer">
                                    ...
                                    <a href="{{ route('viderAu.panier') }}" class="btn btn-outline-secondary"><i
                                            class="fas fa-fw fa-times"></i>
                                        <span>Annuler</span></a>
                                </div>
